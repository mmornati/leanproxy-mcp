...
| `github.list_issues` | 57,949 | 41,020 (−29.2%) | 3,748 (19) | 3,794 (26) |
| `github.search_code` | 41,635 | 33,303 (−20.0%) | 3,784 (36) | 3,769 (44) |
| `jira.jira_search` | 35,069 | 31,350 (−10.6%) | 3,739 (27) | 3,765 (29) |
| `postgres.pg_query` | 45,197 | 45,197 (nothing to drop) | 3,799 (126) | 3,799 (126) |
| **Total** | **234,700** | **205,720 (−12.3%)** | **18,515** | **18,572** |
...
- On top of truncation, the budget is the same, so the token totals are too
  (the projection note adds about 50 tokens per call); what changes is how
  much of the list fits: 26 issues instead of 19, 44 code hits instead of 36.
- With the model's `fields` argument (`[].number`, `[].title`, `[].state`,
  `[].labels[].name`, `[].user.login`, `[].updated_at`), `list_issues`
  shows **65 issues** in 3,785 tokens, against 19 with truncation only.
...
within noise of truncation only.
