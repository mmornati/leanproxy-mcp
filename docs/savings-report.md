...
OpenTelemetry is enabled. Only the deprecated `serve` can also expose them
live, on the JSON `/metrics` endpoint, and only when started with
`--metrics-bind` (off by default; see [Dashboard](dashboard.md#metrics-endpoint)).
You do not need that endpoint: each front end appends a snapshot of
