...
func TestSandboxConfigValidate(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "image starting with a dash is rejected (flag injection)",
			sandbox: &SandboxConfig{Runtime: "docker", Image: "--privileged"},
			command: "npx",
			wantErr: true,
			errMsg:  "is not a valid image reference",
		},
		{
			name:    "image with whitespace is rejected",
			sandbox: &SandboxConfig{Runtime: "docker", Image: "node:22 --privileged"},
			command: "npx",
			wantErr: true,
			errMsg:  "is not a valid image reference",
		},
		{
			name:    "mount container path with extra volume options is rejected",
			sandbox: &SandboxConfig{Runtime: "docker", Mounts: []SandboxMount{{Host: "/home/op/foo", Container: "/work:rw"}}},
			command: "npx",
			wantErr: true,
			errMsg:  "sandbox.mounts[0].container",
		},
		{
			name:    "mount host path with a comma is rejected",
			sandbox: &SandboxConfig{Runtime: "docker", Mounts: []SandboxMount{{Host: "/a,b", Container: "/work"}}},
			command: "npx",
			wantErr: true,
			errMsg:  "sandbox.mounts[0].host",
		},
		{
			name:    "relative mount host path is rejected (would be a named volume)",
			sandbox: &SandboxConfig{Runtime: "docker", Mounts: []SandboxMount{{Host: "foo", Container: "/work"}}},
			command: "npx",
			wantErr: true,
